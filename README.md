...
Imagine a 1-bit nanomechanical computer with 1000 logic gates, as small as physics allows. It has 1 KB RAM and 100 KB disk. All of this fits within a 150-nanometer sphere - roughly as large as a virus - while being duplicated 3 times for radiation hardness. The read-only storage systems form three spirals that gradually twist around the central axis. They turn 180 degrees azimuthally from bottom to top, halting just before reaching the poles. These replicas form an "outer mantle" - geometrically as distant as possible, minimizing the chance that a cosmic ray destroys two copies at once. 

The inner core reaches from zero to half the radius. It contains a shared, radiation-hardened clock, and bidirectional communication channels to each computer. The core signals sensors and actuators whenever 2 replicas produce the same output. The I/O signals travel along both poles and penetrate the crust, a nanometers-thick artificial mitochondrion. The computing system can control a medical robot that travels through the capillaries, then enters any cell in the human body. It performs life-saving surgery with molecular precision and zero collateral damage.
