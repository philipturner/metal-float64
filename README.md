...
- `float59_t` - GPU-friendly format with 15 bits exponent and 48 bits mantissa, with one bit wasted. Must be converted to/from FP64 on the CPU. Throughput ratio is TODO (FMA), ~1:20 (ADD) compared to FP32.
- `float43_t` - GPU-friendly format with 15 bits exponent and 32 bits mantissa, with 17 bits wasted. Must be converted to/from FP64 on the CPU. Throughput ratio is ~1:30-34 (FMA), ~1:15 (ADD) compared to FP32.
